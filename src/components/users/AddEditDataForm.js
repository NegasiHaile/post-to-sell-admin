...
export default function RegisterForm({
  ...
  const formik = useFormik({
    initialValues: {
      ...
      accountStatus: teamMember && teamMember.accountStatus ? teamMember.accountStatus : '',
    ...
    axios
      ...
      .then((res) => {
        if (!res.data.error) {
          ...
          setAddResult({
            state: 'error',
            message: res.data.message || 'Something went wrong while adding products!',
          });
          setIsSubmitting(false);
        }
      })
    ...
    axios
      ...
      .then((res) => {
        if (!res.data.error) {
          ...
          setAddResult({
            state: 'error',
            message: res.data.message || 'Something went wrong while editted products!',
          });
          setIsSubmitting(false);
        }
      })
  ...
  useEffect(() => {
    setFieldValue('fName', teamMember && teamMember.fName ? teamMember.fName : '');
    setFieldValue('lName', teamMember && teamMember.lName ? teamMember.lName : '');
    setFieldValue('email', teamMember && teamMember.email ? teamMember.email : '');
    setFieldValue('role', teamMember && teamMember.role ? teamMember.role : '');
    setFieldValue('accountStatus', teamMember && teamMember.accountStatus ? teamMember.accountStatus : '');
    setFieldValue('telegram', teamMember && teamMember.telegram ? teamMember.telegram : '');
    setFieldValue('whatsup', teamMember && teamMember.whatsup ? teamMember.whatsup : '');
  }, [teamMember]);
                      ...
                      {outerRow.map((innerRow) => {
                        return innerRow.type === 'select' ? (
                          <FormControl fullWidth>
                            <InputLabel id="demo-simple-select-label">{innerRow.label}</InputLabel>
                            <Select
                              size="small"
                              ...
                              onChange={(event) => setFieldValue(innerRow.name, event.target.value)}
                            >
                          ...
                          <TextField
                            ...
                            error={Boolean(touched[innerRow.name] && errors[innerRow.name])}
                            helperText={touched[innerRow.name] && errors[innerRow.name]}
                            disabled={isDisabled}
                          />
                        );
                      })}
